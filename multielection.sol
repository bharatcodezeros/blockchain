...
contract election {
    ...
    function registrationForCandidate(uint _electionNo,string memory _name,string memory _symbol) public returns(bool){
        ...
        for (uint i = 0 ; i < elections[_electionNo].symbols.length;i++){
            if (keccak256(abi.encodePacked(elections[_electionNo].symbols[i])) == keccak256(abi.encodePacked(_symbol))){
                revert("plz add unique symbol");
            }    
        }
        elections[_electionNo].symbols.push(_symbol);
    ...
    function vote(uint _electionNo,address _voterId,string memory _symbol) public{
        ...
        addressOfVoters[_electionNo][_voterId].voted = true;
